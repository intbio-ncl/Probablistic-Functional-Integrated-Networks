 ...
 */
public class DrugSimilarityJaccardIndex {
    public double JaccardDrugSimilarity(String outfile,HashMap<Pair,Integer>CShareddrugs,Map<String,Set<String>>Diseasedrugs)throws IOException{
     ...
     for(String d1: ClusterDisease){
       ...
       for(String i: ClustersPairs.keySet()){
        ...
        try {
            String outFileName = i+"JaccardDrugSimilarity.txt";
            ...
            //System.out.print("before"+ClustersPairs.get(i).size()+"\n");
             for(Pair p:ClustersPairs.get(i)){
                 ...
                 if(CShareddrugs.keySet().contains(p)){
                     ...
                     for(String dis:Diseasedrugs.get(p.getD())){

                          All.add(dis);
}
        for(String dis2:Diseasedrugs.get(p.getG())){
